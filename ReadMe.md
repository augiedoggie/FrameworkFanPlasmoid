...
```
FrameworkFanPlasmoid/
├── install.sh                    # Installer helper script
├── License.md                    # MIT license
├── ReadMe.md                     # This file
├── screenshots/
│   ├── FanProfilesTab.png        # Fan Profiles tab screenshot
│   └── ProfileDetailsTab.png     # Profile Details tab screenshot
└── org.kde.fwfanctrl/
    ├── metadata.json             # Plasma 6 widget metadata
    └── contents/
        ├── config/
        │   ├── config.qml        # Config page declarations
        │   └── main.xml          # KConfig key definitions
        ├── icons/
        │   └── framework.svg     # Widget icon
        └── ui/
            ├── main.qml          # Widget UI & logic
            └── ConfigGeneral.qml # General configuration page
```
...
`main.qml` uses the **Plasma5Support executable DataSource** engine to run
shell commands without spawning a permanent background process:

| Command | Purpose |
|---|---|
| `fw-fanctrl --output-format JSON print` | Retrieves the active strategy, fan speed, and temperature data (polled at the configured interval) |
| `fw-fanctrl --output-format JSON print list` | Retrieves the list of available strategy names on startup |
| `fw-fanctrl use <profile>` | Applies the selected strategy |
| `fw-fanctrl reset` | Resets to the default strategy |
| `fw-fanctrl reload` | Reloads the fw-fanctrl configuration file |
| `fw-fanctrl pause` | Pauses the fw-fanctrl service |
| `fw-fanctrl resume` | Resumes the fw-fanctrl service |

The widget exposes:

- A **compact representation** (tray icon only) for the panel.
- A **full representation** (popup) with a header showing the active profile and two tabs: Available Profiles and Status.
...
| Symptom | Fix |
|---|---|
| Shows "unknown" / "Loading…" forever | Check that `fw-fanctrl` is in `$PATH` for the Plasma session (try `echo $PATH` in a terminal started *from* the session). |
| "fw-fanctrl print failed" | Verify `fw-fanctrl --output-format JSON print` works in a terminal. |
| Icon colour does not change on error or pause | Install `qt6-5compat` (Arch) or `qt6-qt5compat` (Fedora) and restart Plasma. |
| Widget not visible in System Tray config | Run `kpackagetool6 --list \| grep fwfanctrl` to confirm installation, then restart Plasma. |
